...
export default async (req, res, next) => {
	...
	try {
		...
		const exists = await User.exists({ _id: req.user._id, isVerified: true, isActivated: true }).catch((err) => {
			return res.status(500).json(errorHelper("00008", req, err.message));
		});

		if (!exists) return res.status(400).json(errorHelper("00009", req));

		const tokenExists = await Token.exists({ userId: req.user._id, status: true }).catch((err) => {
